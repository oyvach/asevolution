...
int main(int argc, char **argv)
{
	...
	if (argc < 2)
	{
		...
		cout << " catenates particle light-cone output and generates Gadget-2 binaries" << endl
			 << endl;
		...
		cout << "                       in order to avoid negative values (optional, default 0)" << endl
			 << endl;
	...
	for (int i = 1; i < argc; i++)
	{
		...
		case 's':
			...
			break;
		default:
			cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": unknown command-line parameter " << argv[i] << endl
				 << " call lccat without arguments to display help" << endl;
			return -1;
		}
	}
	...
	if (lightconeparam == NULL)
	{
		...
		for (int i = 0; i < sim.num_lightcone; i++)
		{
			if (sim.out_lightcone[i] & MASK_GADGET)
			{
				...
				{
					cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": not all selected light cones refer to the same observation event!" << endl
						 << " incompatible lightcone " << i << " will be omitted!" << endl
						 << endl;
	...
	else
	{
		...
		snprintf(filename,SIZFILENAME, "%s", lightconeparam);
	...
	{
		snprintf(filename,SIZFILENAME, "%s", cycleparam);
	...
	cout << " reading particle headers..." << endl
		 << endl;
	...
	for (int cycle = min_cycle; cycle <= max_cycle; cycle++)
	{
		for (int i = 0; i < sim.num_lightcone; i++)
		{
			...
			if (sim.num_lightcone > 1)
				snprintf(filename,SIZFILENAME, "%s%s%d_%04d_cdm", sim.output_path, sim.basename_lightcone, i, cycle);
			else
				snprintf(filename,SIZFILENAME, "%s%s_%04d_cdm", sim.output_path, sim.basename_lightcone, cycle);
	...
	cout << " " << numread << " particle headers read successfully. Total number of particles = " << numpart_tot << endl
		 << endl;
	...
	cout << " building up particle light cone..." << endl
		 << endl;
			...
			if (infile != NULL)
			{
				...
				for (int64_t p = 0; p < (int64_t)hdr.npartTotal[1] + ((int64_t)hdr.npartTotalHW[1] << 32); p += batch)
				{
					...
					if ((numread == outhdr.npart[1] && numwrite < numfiles - 1) || (uint64_t)numread + numpart_write == numpart_tot)
					{
						...
						{
							posbatch[q] += offset;
							if (posbatch[q] > outhdr.BoxSize)
								outhdr.BoxSize = posbatch[q];
						}

						if (numfiles > 1)
							snprintf(ofilename, "%s%s_cdm.%ld", sim.output_path, sim.basename_lightcone, numwrite);
						else
							snprintf(ofilename, "%s%s_cdm", sim.output_path, sim.basename_lightcone);
	...
	cout << endl
		 << COLORTEXT_GREEN << " particle light cone complete." << endl
	...
	if (outhdr.BoxSize != sim.boxsize / GADGET_LENGTH_CONVERSION)
	{
		...
		{
			snprintf(ofilename, "%s%s_cdm.%d", sim.output_path, sim.basename_lightcone, i);
	...
	cout << endl
		 << COLORTEXT_GREEN << " normal completion." << COLORTEXT_RESET << endl;
