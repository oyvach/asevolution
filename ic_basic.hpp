...
void loadTransferFunctions(const char *filename, gsl_spline *&tk_delta, gsl_spline *&tk_theta, const char *qname, const double boxsize, const double h)
{
	...
	if (parallel.grid_rank()[0] == 0) // read file
	{
		...
		while (!feof(tkfile) && !ferror(tkfile))
		{
			...
			if (line[0] != '#' && !feof(tkfile) && !ferror(tkfile))
				numpoints++;
		}
		...
		while (!feof(tkfile) && !ferror(tkfile))
		{
			...
			for (ptr = line, i = 0; (ptr = strchr(ptr, ':')) != NULL; i++)
			{
				...
				{
					if (strncmp(ptr + 2, qname, strlen(qname)) == 0)
						dcol = i;
				}
				else if (*ptr == 't')
				{
		...
		if (kcol < 0 || dcol < 0 || tcol < 0)
		{
			cout << " kcol dcol tcol: " << kcol << " " << dcol << " " << tcol << endl;
...
double applyMomentumDistribution(Particles<part_simple, part_simple_info, part_simple_dataType> *pcls, unsigned int seed, float T_m = 0., Field<Real> *delta = NULL)
{
	...
	Site x;
	// std::forward_list<part_simple>::iterator it;
	sitmo::prng_engine prng;
	float r1, r2, q, dT;
	double l, dummy, d[3];
	uint32_t i, r;
		...
		if (!pcls->field()(xPart).parts.empty())
		{
			for (auto it = (pcls->field())(xPart).parts.begin(); it != (pcls->field())(xPart).parts.end(); ++it)
			{
				...
				while (true)
				{
					...
					if (q > ql[i + 1] && q < qr[i + 1])
						break;

					if (f[i] + (f[i + 1] - f[i]) * ((float)prng() / (float)sitmo::prng_engine::max()) < q * q / (exp(q) + 1.0f))
						break;
				}
...
void generateIC_basic(metadata &sim, icsettings &ic, cosmology &cosmo, double avgsource, const double fourpiG,
					  ...
					  double *maxvel, Field<Real> *phi, Field<Real> *chi, Field<Real> *Bi, Field<Real> *source,
					  Field<Real> *Sij, Field<Cplx> *scalarFT, Field<Cplx> *BiFT, Field<Cplx> *SijFT, PlanFFT<Cplx> *plan_phi,
					  PlanFFT<Cplx> *plan_chi, PlanFFT<Cplx> *plan_Bi, PlanFFT<Cplx> *plan_source, PlanFFT<Cplx> *plan_Sij,
					  parameter *params, int &numparam)
{
	...
	if (ic.pkfile[0] != '\0') // initial displacements & velocities are derived from a single power spectrum
	{
		...
		generateDisplacementField(*scalarFT, sim.gr_flag * Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource), pkspline, (unsigned int)ic.seed, ic.flags & ICFLAG_KSPHERE);
	}
	else // initial displacements and velocities are set by individual transfer functions
	{
		...
		rescale = 3. * Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource) * (1. + 0.5 * Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource) * ((1. / Hconf(0.98 * a, fourpiG, cosmo, avgsource) / Hconf(0.98 * a, fourpiG, cosmo, avgsource)) - (8. / Hconf(0.99 * a, fourpiG, cosmo, avgsource) / Hconf(0.99 * a, fourpiG, cosmo, avgsource)) + (8. / Hconf(1.01 * a, fourpiG, cosmo, avgsource) / Hconf(1.01 * a, fourpiG, cosmo, avgsource)) - (1. / Hconf(1.02 * a, fourpiG, cosmo, avgsource) / Hconf(1.02 * a, fourpiG, cosmo, avgsource))) / 0.12);
		for (i = 0; i < tk_d1->size; i++) // construct phi
			temp1[i] = (1.5 * (Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource) - pow(sqrt(2. * fourpiG / 3.), 2.) * a * a * cosmo.Omega_Lambda) * tk_d1->y[i] + rescale * tk_t1->y[i] / tk_d1->x[i] / tk_d1->x[i]) * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i];

		if (sim.gr_flag == 0)
		{
			for (i = 0; i < tk_t1->size; i++) // construct gauge correction for N-body gauge (3 Hconf theta_tot / k^2)
				temp2[i] = -3. * Hconf(a, fourpiG, cosmo, avgsource) * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i];
		...
		if (ic.tkfile[0] == '\0')
		{
			if (sim.gr_flag == 0)
			{
				...
				for (i = 0; i < tk_d1->size; i++) // construct gauge correction for N-body gauge velocities
					temp1[i] = -99.5 * Hconf(0.995 * a, fourpiG, cosmo, avgsource) * (3. * temp1[i] * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] + (temp2[i] + 3. * Hconf(0.99 * a, fourpiG, cosmo, avgsource) * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i]));
	...
	{
		rescale = a / Hconf(a, fourpiG, cosmo, avgsource) / (1.5 * Omega_m(a, cosmo) + 2. * Omega_rad(a, cosmo));
	...
	for (p = 0; p < cosmo.num_ncdm; p++)
	{
		...
		{
			rescale = a / Hconf(a, fourpiG, cosmo, avgsource) / (1.5 * Omega_m(a, cosmo) + Omega_rad(a, cosmo));
