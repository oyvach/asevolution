...
struct metric_container
{
	...
	void truncate(double min_dist)
	{
		...
		{
			if (it->second.hdr.distance >= min_dist)
				break;
			else
				free(it->second.pixel);
		}

		if (it != healpix_data.begin())
			healpix_data.erase(healpix_data.begin(), it);
	}
	void clear()
	{
...
int main(int argc, char **argv)
{
	...
	if (argc < 2)
	{
		...
		cout << " extracts local and integrated potential terms from metric light-cone output" << endl
			 << endl;
	...
	cout << COLORTEXT_WHITE << " LCARS tools: lcmap" << endl
		 << endl
		 << " opening settings file of simulation: " << settingsfile << endl
		 << " parser output:" << COLORTEXT_RESET << endl
		 << endl;
	...
	cout << endl
		 << " file contains " << numparam << " parameters, " << usedparams << " of which could be parsed." << endl
		 << endl;
	...
	{
		cout << " lightcone " << i << " parameters:" << endl
			 << "  vertex = " << sim.lightcone[i].vertex[0] << ", " << sim.lightcone[i].vertex[1] << ", " << sim.lightcone[i].vertex[2] << endl;
		...
		cout << "  distance interval = " << sim.lightcone[i].distance[0] << ", " << sim.lightcone[i].distance[1] << endl
			 << endl;
	}

	double tauobs = particleHorizon_old2(1, 1.5 * sim.boxsize * sim.boxsize / C_SPEED_OF_LIGHT / C_SPEED_OF_LIGHT, cosmo);

	FILE *background_file;
	char *buffer;

	int numlines = -2;
	snprintf(filename0, SIZPARSTRING, "%s%s_background.dat", sim.output_path, sim.basename_generic);
	background_file = fopen(filename0, "r");

#ifdef DEBUG
	COUT << "Reading background file\n";
#endif
	...
	while (!feof(background_file))
	{
		...
		{
			cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": read error! 1" << endl;
			return 0;
		}
		numlines++;
	}
	...
	if (fscanf(background_file, "%*[^\n]\n") != 0)
	{
		cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": read error! 2" << endl;
		return 0;
	}
	if (fscanf(background_file, "%*[^\n]\n") != 0)
	{
		cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": read error! 3" << endl;
		return 0;
	}

	background_data back[numlines];
#ifdef DEBUG
        COUT << "Reached reading: numlines is "<< numlines << endl;
#endif
...
#endif
#ifdef GW
		if (fscanf(background_file, "      %i   %lf   %lf   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s\n",
...
#else
                if (fscanf(background_file, "      %i   %lf   %lf   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s   %*s\n",
                                   &back[numlines - 1 - i].cycle, &back[numlines - 1 - i].tau, &back[numlines - 1 - i].a) != 3)
		{
			cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": read error! " << endl;
			return 0;
...
#ifdef DEBUG
	COUT << "\nFinished reading background file\n";
#endif
	...
	{
		snprintf(filename0, SIZPARSTRING, "%s", distanceparam);
	...
	{
		snprintf(filename0, SIZPARSTRING, "%s", redshiftparam);
	...
	if (redshiftparam == NULL && distanceparam != NULL)
	{
		...
		qsort((void *)distances, numoutputs, sizeof(double), [](const void *a, const void *b) -> int
	...
	else if (redshiftparam != NULL && distanceparam == NULL)
	{
		...
		qsort((void *)redshifts, numoutputs, sizeof(double), [](const void *a, const void *b) -> int
			  { return (int)(*((double *)a) > *((double *)b)); });
		...
		for (int i = 0; i < numoutputs; i++)
		{
			distances[i] = tauobs - particleHorizon_old2(1. / (redshifts[i] + 1.), 1.5 * sim.boxsize * sim.boxsize / C_SPEED_OF_LIGHT / C_SPEED_OF_LIGHT, cosmo);
...
#ifdef DEBUG
	COUT << "Reached field declarations\n";
#endif
	...
	cout << COLORTEXT_YELLOW << " processing light-cone output..." << COLORTEXT_RESET << endl
		 << endl;
	...
	{
		cout << " interpolating cycle number " << COLORTEXT_CYAN << back[step].cycle << COLORTEXT_RESET << " and " << COLORTEXT_CYAN << back[step + 1].cycle << COLORTEXT_RESET << "." << endl
			 << " distance interval: " << (tauobs - back[step].tau) * sim.boxsize << " to " << (tauobs - back[step + 1].tau) * sim.boxsize << endl
			 << endl;
...
#pragma omp parallel for
		...
		for (; it0->second.hdr.distance < tauobs - back[step + 1].tau && it1->second.hdr.distance < tauobs - back[step + 1].tau && dist < distances[numoutputs - 1]; it0++, it1++)
		{
			...
			if (it0->second.hdr.Nside != Nside_final)
			{
				...
				cout << ": from Nside of " << COLORTEXT_WHITE << Nside_final << COLORTEXT_RESET << " to " << COLORTEXT_WHITE << it0->second.hdr.Nside << COLORTEXT_RESET << "." << endl
					 << endl;
...
#pragma omp parallel for
			...
			{
				cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": map properties do not match on iteration " << cnt << "! distances: " << it0->second.hdr.distance << ", " << it1->second.hdr.distance << "; Nside: " << it0->second.hdr.Nside << ", " << it1->second.hdr.Nside << "; Npix: " << it0->second.hdr.Npix << ", " << it1->second.hdr.Npix << endl
					 << endl;
				return -1;
			}
...
#pragma omp parallel for
			...
			{
				cout << " source distance reached of " << distances[outcnt] * sim.boxsize << " Mpc/h!" << endl
					 << endl;
...
#pragma omp parallel for
				...
				cout << " computing convergence..." << endl
					 << endl;
...
#pragma omp parallel for
				...
				if (numoutputs > 1)
					snprintf(outputfile, SIZPARSTRING, "%s%s_kappa_%d.fits", sim.output_path, sim.basename_lightcone, outcnt);
				else
					snprintf(outputfile, SIZPARSTRING, "%s%s_kappa.fits", sim.output_path, sim.basename_lightcone);
				write_healpix_map(map_kappa_final + pixoffset, Nside_final, outputfile, 0, &coordsys);

				free(map_kappa_final);

				if (numoutputs > 1)
					snprintf(outputfile, SIZPARSTRING, "%s%s_lensingphi_%d.fits", sim.output_path, sim.basename_lightcone, outcnt);
				else
					snprintf(outputfile, SIZPARSTRING, "%s%s_lensingphi.fits", sim.output_path, sim.basename_lightcone);
...
#pragma omp parallel for
				...
				if (numoutputs > 1)
					snprintf(outputfile, SIZPARSTRING, "%s%s_isw_%d.fits", sim.output_path, sim.basename_lightcone, outcnt);
				else
					snprintf(outputfile, SIZPARSTRING, "%s%s_isw.fits", sim.output_path, sim.basename_lightcone);
				write_healpix_map(map_phi_final[outcnt] + pixoffset, Nside_final, outputfile, 0, &coordsys);
...
#pragma omp parallel for
				...
				if (numoutputs > 1)
					snprintf(outputfile, SIZPARSTRING, "%s%s_potential_%d.fits", sim.output_path, sim.basename_lightcone, outcnt);
				else
					snprintf(outputfile, SIZPARSTRING, "%s%s_potential.fits", sim.output_path, sim.basename_lightcone);
				write_healpix_map(map_phi_final[outcnt] + pixoffset, Nside_final, outputfile, 0, &coordsys);
...
#pragma omp parallel for
				...
				if (numoutputs > 1)
					snprintf(outputfile, SIZPARSTRING, "%s%s_shapiro_%d.fits", sim.output_path, sim.basename_lightcone, outcnt);
				else
					snprintf(outputfile, SIZPARSTRING, "%s%s_shapiro.fits", sim.output_path, sim.basename_lightcone);
				write_healpix_map(map_phi_final[outcnt] + pixoffset, Nside_final, outputfile, 0, &coordsys);
...
int loadHealpixData(metric_container *field, double min_dist, double max_dist, char *lightconeparam)
{
	...
	if (lightconeparam != NULL)
	{
		...
		do
		{
			snprintf(filename, SIZPARSTRING, "%s%s%d_%04d_%s.map", field->dir, field->basename, atoi(token), field->cinfo.cycle, field->name);
	...
	{
		snprintf(filename, SIZPARSTRING, "%s%s_%04d_%s.map", field->dir, field->basename, field->cinfo.cycle, field->name);
		infile = fopen(filename, "rb");
	}
	...
	for (count = 0; !feof(infile) && !ferror(infile); count++)
	{
		...
		if (metric.hdr.distance > min_dist)
			break;
...
#pragma omp parallel for private(j)
		...
		if (metric.hdr.distance > max_dist)
			break;
...
bool kappa(float *pixel, const int64_t Nside, int64_t ipix, float &result)
{
	...
	else // south polar cap
	{
		...
		if (ring == Nside)
		{
			...
			if (pixel[12l * Nside * Nside - 1 - k] < -1e30 || pixel[12l * Nside * Nside - 1 - l] < -1e30)
				return false;
		...
		if (ring == 1)
		{
			if (pixel[12l * Nside * Nside - 1] < -1e30 || pixel[12l * Nside * Nside - 2] < -1e30 || pixel[12l * Nside * Nside - 3] < -1e30 || pixel[12l * Nside * Nside - 4] < -1e30)
				return false;
