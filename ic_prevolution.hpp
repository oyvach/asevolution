...
void generateIC_prevolution(metadata &sim, icsettings &ic, cosmology &cosmo, double avgsource, const double fourpiG, double &a, double &tau, double &dtau, double &dtau_old, Particles<part_simple, part_simple_info, part_simple_dataType> *pcls_cdm, Particles<part_simple, part_simple_info, part_simple_dataType> *pcls_b, Particles<part_simple, part_simple_info, part_simple_dataType> *pcls_ncdm, double *maxvel, Field<Real> *phi, Field<Real> *chi, Field<Real> *Bi, Field<Real> *source, Field<Real> *Sij, Field<Cplx> *scalarFT, Field<Cplx> *BiFT, Field<Cplx> *SijFT, PlanFFT<Cplx> *plan_phi, PlanFFT<Cplx> *plan_chi, PlanFFT<Cplx> *plan_Bi, PlanFFT<Cplx> *plan_source, PlanFFT<Cplx> *plan_Sij, parameter *params, int &numparam)
{
#ifdef HAVE_CLASS
	int i, j, p;
	float *pcldata = NULL;
	...
	a = 1. / (1. + ic.z_ic);
	tau = particleHorizon_old(a, fourpiG, cosmo, avgsource);
	...
	for (p = 0; p < cosmo.num_ncdm; p++)
	{
		...
		snprintf(filename,SIZFILENAME, "_dT_ncdm%d.h5", p);
	...
	do
	{
		if (cycle > 0 && (1. / a < ic.z_relax + 1. || 1. / a <= sim.z_in + 1.))
		{
			...
			prepareFTsource<Real>(*phi, *chi, *source, cosmo.Omega_cdm + cosmo.Omega_b, *source, 3. * Hconf(a, fourpiG, cosmo, avgsource) * dx * dx / dtau_old, fourpiG * dx * dx / a, 3. * Hconf(a, fourpiG, cosmo, avgsource) * Hconf(a, fourpiG, cosmo, avgsource) * dx * dx);
			plan_source->execute(FFT_FORWARD);
			solveModifiedPoissonFT(*scalarFT, *scalarFT, 1. / (dx * dx), 3. * Hconf(a, fourpiG, cosmo, avgsource) / dtau_old);
		...
		if (relax_cycles == 1)
		{
			...
			snprintf(filename,SIZFILENAME, "%s%sICtarget_phi.dat", sim.output_path, sim.basename_pk);
			...
			plan_chi->execute(FFT_FORWARD);
			extractPowerSpectrum(*scalarFT, kbin, power, kscatter, pscatter, occupation, sim.numbins, false, KTYPE_LINEAR);
			snprintf(filename,SIZFILENAME, "%s%sICactual_phi.dat", sim.output_path, sim.basename_pk);
			writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real)numpts3d * (Real)numpts3d * 2. * M_PI * M_PI, filename, "power spectrum of phi", a);
		}
		...
		if (cycle == 0)
		{
			snprintf(filename,SIZFILENAME, "_ICinit_phi.h5");
			phi->saveHDF5(h5filename + filename);

			plan_phi->execute(FFT_FORWARD);
			extractPowerSpectrum(*scalarFT, kbin, power, kscatter, pscatter, occupation, sim.numbins, false, KTYPE_LINEAR);
			snprintf(filename,SIZFILENAME, "%s%sICinit_phi.dat", sim.output_path, sim.basename_pk);
			writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real)numpts3d * (Real)numpts3d * 2. * M_PI * M_PI, filename, "power spectrum of phi", a);
		}
		...
		if (relax_cycles > 0 && ic.Cf > sim.Cf)
			dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx + (a - (1. / (ic.z_relax + 1.))) * ((sim.Cf * dx < sim.steplimit / Hconf(a, fourpiG, cosmo, avgsource)) ? (sim.Cf * dx) : (sim.steplimit / Hconf(a, fourpiG, cosmo)))) / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
		else if (relax_cycles > 0 && ic.Cf * dx > sim.steplimit / Hconf(a, fourpiG, cosmo, avgsource))
			dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx + (a - (1. / (ic.z_relax + 1.))) * sim.steplimit / Hconf(a, fourpiG, cosmo, avgsource)) / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
		else
			dtau = ic.Cf * dx;
	...
	if (sim.gr_flag == 0)
	{
		...
		rescale = 3. * Hconf(a, fourpiG, cosmo, avgsource) * M_PI;
		...
		rescale = 3. * Hconf(0.99 * a, fourpiG, cosmo, avgsource) * M_PI;
		mean_q = -99.5 * Hconf(0.995 * a, fourpiG, cosmo, avgsource);
