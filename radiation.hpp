...
void projection_T00_project(background &class_background, perturbs &class_perturbs, Field<Real> &source, Field<Cplx> &scalarFT, PlanFFT<Cplx> *plan_source, metadata &sim, icsettings &ic, cosmology &cosmo, double avgsource, const double fourpiG, double a, double coeff = 1.)
{
	gsl_spline *tk1 = NULL;
	gsl_spline *tk2 = NULL;
	double *delta = NULL;
	...
	if (n > 0)
	{
		if (sim.gr_flag == 0) // add gauge correction for N-body gauge
		{
			...
			rescale = Hconf(a, fourpiG, cosmo, avgsource);
...
void prepareFTchiLinear(background &class_background, perturbs &class_perturbs, Field<Cplx> &scalarFT, metadata &sim, icsettings &ic, cosmology &cosmo, double avgsource, const double fourpiG, double a, double coeff = 1.)
{
	gsl_spline *tk1 = NULL;
	gsl_spline *tk2 = NULL;
	double *chi = NULL;
	...
	if (sim.gr_flag == 0) // add gauge correction for N-body gauge
	{
		...
		double Hconf1 = Hconf(0.99 * a, fourpiG, cosmo, avgsource);
		double Hconf2 = Hconf(0.995 * a, fourpiG, cosmo, avgsource);
		double Hconf3 = Hconf(a, fourpiG, cosmo, avgsource);
		double Hconf4 = Hconf(1.005 * a, fourpiG, cosmo, avgsource);
		double Hconf5 = Hconf(1.01 * a, fourpiG, cosmo, avgsource);
